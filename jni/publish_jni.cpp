...
Java_com_blueberry_media_PublishJni_nativePush(JNIEnv *env,jclass type,jlong cptr,
                                                jbyteArray h264_raw_,jint length,jlong timestamp){
    jbyte *h264_raw = env->GetByteArrayElements(h264_raw_,NULL);

    Rtmp *rtmp = reinterpret_cast<Rtmp *>(cptr);
    int ret = rtmp->doNativePush((BYTE *)h264_raw, length, timestamp);

    env->ReleaseByteArrayElements(h264_raw_,h264_raw,0);
    return ret;
}
