...
int Rtmp::init(std::string url, int w, int h, int timeOut) {
    ...
    pthread_t tid;
    mThreadRun = 1;
    int ret = pthread_create(&tid, NULL,Rtmp::threadHandle,(void*)this);
...
void * Rtmp::threadHandle(void* arg) {
    Rtmp *ptr = (Rtmp*)arg;

    int type, length, ret;
    long timestamp;
    while(ptr->mThreadRun){
        if(!ptr->mVideoQueue.empty()){
            struct videoRawData *rawData = ptr->mVideoQueue.front();
            ptr->mVideoQueue.pop();

            BYTE * h264_raw = rawData->data;
            length = rawData->data_length;
            timestamp = rawData->timestamp;

            /*判断nal type*/
            if (h264_raw[2] == 0x00) {/*00 00 00 01*/
                type = h264_raw[4] & 0x1f;
            } else if (h264_raw[2] == 0x01) {
                type = h264_raw[3] & 0x1f;
            }

            if (type == NAL_SPS){//sps and pps 一起发送

                BYTE *pps = (BYTE *)malloc(5);
                BYTE *sps = (BYTE *)malloc(length - 13);

                h264_raw += 4;//抛弃0，0，0，1
                memcpy(sps, h264_raw, length-13);
                h264_raw += length - 13;
                h264_raw += 4;
                memcpy(pps, h264_raw, 5);
                ret = ptr->sendSpsAndPps(sps, length - 13, pps, 5, timestamp);

                free(sps);
                free(pps);
            }else{

                ret = ptr->sendVideoData(h264_raw, length, timestamp);
            }
...
int Rtmp::doNativePush(BYTE * h264_raw, int length, long timestamp){

    struct videoRawData *rawData;
    rawData->data = (BYTE *)malloc(length);
    memcpy(rawData->data, h264_raw, length);

    rawData->data_length = length;
    rawData->timestamp = timestamp;

    mVideoQueue.push(rawData);
}
